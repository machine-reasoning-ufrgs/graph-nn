...
#		
	...
	#Train gnn and degree_mlp only (using degree_cost)
	tvars_ = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="GraphNN") + tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="degree_MLP")
	vars_cost_ = tf.zeros([])
	for v in tvars_ :
		vars_cost_ = tf.add( vars_cost_, tf.nn.l2_loss( v ) ) #tf.Print(vars_cost, [v.name], "Var name:")
	
	loss = tf.add_n( [ degree_predict_cost, tf.multiply( vars_cost_, parameter_l2norm_scaling ) ] )
	grads, _ = tf.clip_by_global_norm( tf.gradients( loss, tvars_ ), global_norm_gradient_clipping_ratio )
	train_step = optimizer.apply_gradients( zip( grads, tvars_ ) )
	

#		grads_degree, _ = tf.clip_by_global_norm( tf.gradients( degree_predict_cost, tvars ), global_norm_gradient_clipping_ratio )
#		train_step = optimizer.apply_gradients( zip( grads_degree, tvars ) )
