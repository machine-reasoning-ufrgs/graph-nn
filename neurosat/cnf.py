...
class CNF(object):
	...
	def read_dimacs(path):
		with open(path,"r") as f:
			...
			cnf.sat = bool(sat)
			for i in range(m):
				cnf.clauses[i] = [ int(x) for x in f.readline().split()[:-1]]
			#end
		#end
		return cnf
	#end
	...
	def matrix_to_CNF(M):
		"""
			Converts a n×m binary matrix into a n variable, m clause CNF formula
		"""
	...
	def is_sat(self):
		if self.sat != None:
			return self.sat
		elif pycosat.solve(self.clauses) == "UNSAT":
			return False
		else:
			return True
		#end
...
def create_dataset( n_min = 10, n_max = 40, samples = 1000, path = "instances" ):
	...
	if not os.path.exists("{}/unsat/".format(path)):
		os.makedirs("{}/unsat/".format(path))
	#end if

	if not os.path.exists("{}/sat/".format(path)):
		os.makedirs("{}/sat/".format(path))
	#end if
