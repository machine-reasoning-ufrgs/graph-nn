...
def build_network(d):
	...
	"""
		logits is prepared as: route_edges + (1-route_edges) .* E_vote
		such that:
			logits[i] = E_vote[i] when route_edges[i] = 0 and
	...
	edges_loss = tf.reduce_sum([pos_edges_loss, neg_edges_loss])
